...
(defn to-rails-date-string [datetime]
...
(fact "can convert a map to a map with a Jodatime timestamp"
  (let [input-date (time/date-time 2002 3 4)
        input-string (to-rails-date-string input-date)
        input-map {:timestamp input-string
                   :other-key "other value"}]
    (convert-embedded-timestamp input-map) => {:timestamp input-date
                                               :other-key "other value"}))
